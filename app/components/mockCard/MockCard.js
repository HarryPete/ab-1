    ...
    return(
        <div className={styles.container}>
            <div className={styles.header}>
                ...
                <p className={styles.mock}>{mock.assessment.length} questions</p>
            </div>
            <p className={styles.type}>{mock.type}</p>
            <div className={styles.footer}>
                <p className={styles.date}>{FormatDate(mock.createdAt).split(',')[0]}</p>
                <div className={styles.nav}>
                    <Button text={mock.result.length ? 'Retake' : 'Start'} route={`/dashboard/mock/${mock._id}`}/>
                    {mock.result.length > 0 && <Button text='Review' route={`/dashboard/review/${mock._id}`}/>}
                </div>
            </div>
        </div>
    )
